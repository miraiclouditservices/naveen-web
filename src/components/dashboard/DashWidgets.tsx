...
export function StatCard({ label, value, icon, color, sub, trend }: any) {
  return (
    <div style={{ background:"var(--bg-card)",borderRadius:12,border:"1px solid var(--border-color)",padding:"16px 18px",
      ...
      <div style={{ flex:1,minWidth:0 }}>
        <div style={{ fontSize:"0.62rem",fontWeight:800,color:"var(--text-muted)",textTransform:"uppercase",letterSpacing:"0.06em" }}>{label}</div>
...
// ── Progress Bar ──────────────────────────────────────────────────────────────
export function ProgressBar({ label, val, max, color="var(--dark-section)", showVal=true }: any) {
  const pct = max > 0 ? Math.min(100, Math.round((val/max)*100)) : 0;
  return (
    <div style={{ marginBottom:10 }}>
      <div style={{ display:"flex",justifyContent:"space-between",fontSize:"0.74rem",fontWeight:600,color:"var(--text-primary)",marginBottom:4 }}>
        <span>{label}</span>
        {showVal && <span style={{ color }}>{pct}%</span>}
      </div>
      <div style={{ background:"var(--border-color)",borderRadius:8,height:8,overflow:"hidden" }}>
...
export function BarChart({ items, height=100, color="var(--dark-section)" }: { items:{label:string;value:number}[];height?:number;color?:string }) {
      ...
      <div style={{ display:"flex",alignItems:"flex-end",gap:6,height,paddingTop:8 }}>
        {items.map((item,i)=>(
          <div key={i} style={{ flex:1,display:"flex",flexDirection:"column",alignItems:"center",gap:4 }}>
            <div style={{ fontSize:"0.58rem",color:"var(--text-muted)",fontWeight:700,minHeight:14 }}>
            ...
            <div style={{ fontSize:"0.58rem",color:"var(--text-muted)",fontWeight:600,textAlign:"center",whiteSpace:"nowrap" }}>{item.label}</div>
          </div>
        ))}
      </div>
    </div>
...
export function LineChart({ items, color="var(--dark-section)", height=80 }: { items:{label:string;value:number}[];color?:string;height?:number }) {
...
// ── Donut Chart (CSS) ─────────────────────────────────────────────────────────
export function DonutGauge({ pct, color="var(--dark-section)", label, size=100 }: any) {
  ...
  return (
    <div style={{ display:"flex",flexDirection:"column",alignItems:"center",gap:6 }}>
      ...
      {label && <div style={{ fontSize:"0.7rem",color:"var(--text-muted)",fontWeight:600,textAlign:"center" }}>{label}</div>}
    </div>
  );
}
...
const PILL_CFG: Record<string,{bg:string;cl:string}> = {
  ...
  "Checked-Out":{bg:"var(--border-color)",cl:"var(--text-primary)"},Active:{bg:"#dcfce7",cl:"#166534"},
  Expired:{bg:"#fee2e2",cl:"#991b1b"},Cleared:{bg:"#dbeafe",cl:"#1e40af"},
};
export function StatusPill({ status }: { status:string }) {
  const s = PILL_CFG[status]||{bg:"var(--border-color)",cl:"var(--text-primary)"};
  ...
  return (
    <div style={{ background:`linear-gradient(135deg,${gradientFrom} 0%,${gradientTo} 100%)`,
      ...
      <div style={{ flex:1,minWidth:"280px" }}>
        ...
        <h2 style={{ color:"var(--bg-card)",fontWeight:800,fontSize:"1.5rem",margin:0 }}>{title}</h2>
        <p style={{ color:"var(--text-muted)",fontSize:"0.78rem",margin:"4px 0 0" }}>{subtitle}</p>
        {children}
      </div>
      <div style={{ display:"flex",gap:10,alignItems:"center" }}>
        <div style={{ textAlign:"right" }}>
          <div style={{ color:"var(--bg-card)",fontWeight:700,fontSize:"0.9rem" }}>{user?.name||"User"}</div>
        ...
        <div style={{ width:42,height:42,borderRadius:"50%",background:accentColor,display:"flex",alignItems:"center",justifyContent:"center" }}>
          <i className="bi bi-person-fill" style={{ color:"var(--bg-card)",fontSize:"1.2rem" }} />
        </div>
      </div>
    </div>
  );
...
export function AlertItem({ icon, iconColor, title, sub, time }: any) {
  return (
    <div style={{ display:"flex",gap:10,alignItems:"flex-start",padding:"9px 0",borderBottom:"1px solid var(--border-color)" }}>
