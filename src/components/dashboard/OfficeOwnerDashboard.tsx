...
export default function OfficeOwnerDashboard({ user }: { user: any }) {
  ...
  useEffect(()=>{
    Promise.all([
    ...
    ]).then(([visitorsRes, helpdeskRes, materialsRes, bookingsRes, meRes, metricsRes]) => {
      ...
      if (metricsRes.success) setAgreementM(metricsRes.data.agreementMetrics || {});
    }).catch(err => {
  ...
  return (
    <div style={{ paddingBottom:40 }}>
      ...
      {/* ── Agreement & Financial Status ── */}
      <h3 style={{ fontSize: "1.1rem", fontWeight: 800, color: "#1e293b", margin: "20px 0 14px", textTransform: "uppercase", letterSpacing: "0.04em" }}>
        Agreement & Financial Status
      </h3>
      <div className="d-g4 d-mb">
        <StatCard 
          label="Agreement Amount" 
          value={`₹${(agreementM.agreementAmount || 0).toLocaleString()}`} 
        ...
        <StatCard 
          label="Paid Amount" 
          value={`₹${(agreementM.paidAmount || 0).toLocaleString()}`} 
          icon="bi-check2-circle" 
          color={C.green} 
          sub="Total payments made" 
        />
        <StatCard 
          label="Pending Amount" 
          value={`₹${(agreementM.pendingAmount || 0).toLocaleString()}`} 
          icon="bi-clock" 
          color={C.yellow} 
        ...
        <StatCard 
          ...
          value={agreementM.nextPaymentDate ? new Date(agreementM.nextPaymentDate).toLocaleDateString() : "—"} 
          icon="bi-calendar-event" 
          color={C.red} 
          sub="Payment due date" 
        />
      </div>
